...
try:
    from pedalboard import Pedalboard, Reverb, Compressor, HighShelfFilter, PeakingFilter, HighpassFilter, Gain
except ImportError:
...
def apply_spectral_depresence(audio, sample_rate, intensity, presence_override=None):
    ...
    pb = Pedalboard([
        ...
        # PeakingFilter: cutoff, gain, q
        PeakingFilter(cutoff_frequency_hz=3000, gain_db=-4.0 * p_int, q=1.2),
        HighShelfFilter(cutoff_frequency_hz=6000, gain_db=-3.0 * intensity),
        PeakingFilter(cutoff_frequency_hz=300, gain_db=1.5 * intensity, q=0.8),
...
def apply_reverb(audio, sample_rate, wet, dry=0.75):
    pb = Pedalboard([
        Reverb(room_size=0.5, damping=0.7, wet_level=wet, dry_level=dry)
    ])
    return pb(audio.T, sample_rate).T
...
def create_ab_comparison(original, processed, sample_rate, segment_duration=10.0, crossfade_duration=0.5):
...
def main():
    parser = argparse.ArgumentParser(description="Process audio to sound like background music.")
    parser.add_argument("input", help="Input audio file")
    parser.add_argument("output", help="Output audio file")
    parser.add_argument("--intensity", type=float, default=0.7, help="Master control 0.0-1.0")
    parser.add_argument("--width", type=float, default=None, help="Stereo width override 0.0-1.0")
    parser.add_argument("--reverb-wet", type=float, default=None, help="Reverb wet mix override 0.0-1.0")
    parser.add_argument("--presence", type=float, default=None, help="Presence cut override 0.0-1.0")
    ...
    parser.add_argument("--ab", action="store_true", help="Output a 30-second A/B comparison file")
